    ...
    <div className="flex min-h-screen bg-background">
      ...
      <div className="flex-1 flex flex-col min-w-0 lg:ml-48">
        ...
        <header className="h-14 border-b bg-background flex items-center px-4 lg:px-8 sticky top-0 z-30">
          ...
          <span className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground bg-muted px-2 py-1 rounded shrink-0 ml-3">
            {role === 'admin' ? 'Admin' : role === 'user' ? 'Staff' : navItems.some(item => item.href.startsWith('/admin')) ? 'Admin' : 'Partner'}
          </span>
        </header>
